...
Decomposition belongs to [triage](issue-triage.md). It leaves tracking parents
untriaged and releases prepared children into normal triage, each with its own
issue number, tracking label, branch, and PR. The implementation workflow is
unchanged and has no extra decomposition gate: manually adding `triaged` to a
parent or pending child bypasses triage's handoff decision. Do not use that as
a recovery mechanism; comment on the parent instead.
