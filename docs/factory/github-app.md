...
`<app-slug>[bot]` identity. The [review workflow](pr-review.md) uses the separate
`factory-reviewer-bot[bot]` App, allowing it to approve Factory PRs without violating
...
## Configure the reviewer App

Use a different App from the PR-authoring Factory App:

1. Register `factory-reviewer-bot` with repository **Contents: read-only** and
   **Pull requests: read and write**. Metadata read is automatic; leave other
   permissions unset. Disable webhooks; no webhook server, callback URL, or
   event subscriptions are needed for installation-token authentication.
2. Install it on this repository and approve its requested permissions.
3. Save its Client ID as Actions variable `FACTORY_REVIEWER_CLIENT_ID`.
4. Save its complete private-key PEM as Actions secret
   `FACTORY_REVIEWER_PRIVATE_KEY`. Do not post the key or tokens in conversations
   or commit them. Keep the existing Factory variable and secret unchanged.

The review workflow requests only these installation permissions and verifies
the authenticated login against its `REVIEWER_LOGIN`
(`factory-reviewer-bot[bot]`) before review. A missing credential, denied grant,
or wrong App fails setup rather than falling back to the built-in token.
The existing `FACTORY_CLIENT_ID` / `FACTORY_PRIVATE_KEY` continue to select the
implementation/triage App.
...
The reviewer App has no Actions-write grant. Its submitted reviews trigger the
existing native router event; no completion-notification job or second review
...
Token creation, identity verification, and receipt-check credentials remain in
the workflows.
...
The reviewer uses its own App token for both review submission and receipt
verification; the built-in token is only for checkout and Copilot model access.
App approvals require the installation's Pull requests write grant and remain
subject to repository review policies. The workflow does not require
`GITHUB_TOKEN` permission to create or approve PRs.

Using the same App identity to create and review PRs reintroduces the self-approval
problem. Use `COMMENT`, not `APPROVE`, when a PR's author equals `REVIEWER_LOGIN`.
Existing PRs retain their original author. Factory App PR creation has been
tested successfully; reviewer-App authentication, approvals, and native event
delivery still need the live verification tracked in the [review guidance](pr-review.md).
