...
- Review the full checked-out repository snapshot from scratch on **every
  invocation, including the first and retries**.
...
- Prior findings and runs inform duplicate checks, not review coverage. Do not
  restrict analysis to a diff, a time window, or previously reported problems.
- This is source review, not [workflow diagnostics](workflow-diagnostics.md).
